...
export class StepService {
    ...
    public async createStep(data:CreateStepDto):Promise<CreateStepDto>{
        const verifyPeriodusIdExist = await this.prismaService.periodo.findUnique({ where: { id: data.id_periodo } })

        if (!verifyPeriodusIdExist) {
            throw new NotFoundException(`Etapa com ID ${data.id_periodo} não encontrada.`);
        }
